...
`tigris` (globally-distributed S3-compatible object storage: objects under a
`scatterbox/` key prefix in a bucket, single-PUT uploads, key-addressed
objects; authenticates with an S3 access key/secret signed with AWS SigV4 —
not OAuth — static, so a rejected key is a re-auth rather than a refresh. The
endpoint is fixed (one global endpoint), so only the non-secret bucket is
...
| `provider add NAME --type localfs\|gdrive\|onedrive\|dropbox\|pcloud\|koofr\|tigris …` | Onboard (OAuth for cloud types; app password for koofr; S3 key/secret for tigris) |
