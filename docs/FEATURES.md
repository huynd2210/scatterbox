...
320 KiB-aligned fragments). `chaos` exists for tests only (failure
injection: 404s, corruption, latency, hard-kill).
...
| `provider add NAME --type localfs\|gdrive\|onedrive …` | Onboard (OAuth flow for cloud types) |
