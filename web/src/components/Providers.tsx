...
export function Providers({ refreshKey }: { refreshKey: number }) {
  ...
  const reauth = (p: ProviderInfo, askCreds = false) => {
    ...
    if (p.type === "koofr") {
      ...
      // the namespace/region/bucket on the register row are unchanged.
      const accessKeyId = prompt(`Oracle Access Key for ${p.name}`);
      if (!accessKeyId) return;
      const secretAccessKey = prompt("Oracle Secret Key");
