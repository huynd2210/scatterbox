...
def onboard_secret_provider(
    ...
    *,
    ...
    not discover at runtime (an S3 backend's bucket / endpoint); it is merged
    into the register row, never the vault."""
    _ensure_name_free(register, name)
    return _store_test_register(
...
def _store_test_register(
    ...
    *,
    ...
    extra_config (non-secret, e.g. an S3 bucket) is merged into the register
    row alongside the secret reference."""
