"""Tigris adapter against a fake S3 endpoint.

Tigris speaks the S3 REST API with AWS SigV4 auth at a single global endpoint,
...
from scatterbox.providers import tigris
...
def make_provider(fake: FakeS3, **kwargs) -> TigrisProvider:
    secrets = FakeSecrets(s=tigris.credential_blob(AKID, SECRET))
...
def test_uses_the_fixed_global_endpoint(fake):
    p = make_provider(fake)
    asyncio.run(p.put("c", b"x"))
    # default endpoint is Tigris's single global host
    assert fake.requests[-1].url.host == "fly.storage.tigris.com"
...
def test_missing_credential_blob_is_a_clear_error():
    ...
    with pytest.raises(ScatterboxError, match="access key/secret"):
        TigrisProvider(secrets=secrets, secret_name="s", bucket=BUCKET)
