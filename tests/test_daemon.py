...
def test_tigris_provider_add_via_api(client, monkeypatch):
    """Tigris is secret-backed but not OAuth: the endpoint takes an S3 access
    key/secret plus the (non-secret) bucket, and hands the shared flow the key
    blob and bucket extra_config, no browser."""
    from scatterbox import onboarding
    ...
    unlock(client)
    # missing keys rejected up front (bucket present, no key/secret)
    assert (
        client.post(
            "/api/providers", json={"name": "tg", "type": "tigris", "bucket": "buck"}
        ).status_code
        == 400
    )
    ...
    assert resp.status_code == 200, resp.text
    assert calls["name"] == "tg" and calls["type"] == "tigris"
    assert calls["blob"] == {"access_key_id": "AKID", "secret_access_key": "SEC"}
    assert calls["extra_config"] == {"bucket": "buck"}
