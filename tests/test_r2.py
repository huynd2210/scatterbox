"""Cloudflare R2 adapter against a fake S3 endpoint.

R2 speaks the S3 REST API with AWS SigV4 auth, so the fake is a path-style S3
bucket over httpx.MockTransport: object PUT/GET/HEAD/DELETE plus a ListObjectsV2
for quota. It validates the adapter's orchestration and content round-trip; the
SigV4 signing itself is pinned independently against AWS's published test
vector (test_sigv4_matches_aws_known_answer) so the offline round-trip is more
...
from scatterbox.providers import r2
...
class FakeS3:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        # Every request must carry a SigV4 Authorization for our access key and
        # the payload hash header — proof the signing path is wired.
...
def make_provider(fake: FakeS3, **kwargs) -> R2Provider:
    secrets = FakeSecrets(s=r2.credential_blob(AKID, SECRET))
    return R2Provider(
        secrets=secrets,
        secret_name="s",
        account_id=ACCOUNT,
...
def test_sigv4_matches_aws_known_answer():
    # AWS SigV4 test-suite "get-vanilla": the canonical fixed credentials, date,
    # region "us-east-1", and service "service" must yield this exact signature.
    # Pins our signing implementation to the spec, not just to itself.
    auth = sign_v4(
        method="GET",
        canonical_uri="/",
        canonical_query="",
...
def test_rejected_keys_surface_reauth_error():
    # A 403 (SignatureDoesNotMatch / InvalidAccessKeyId) cannot be refreshed —
    # the adapter turns it into a clear re-authenticate error.
    transport = httpx.MockTransport(
        lambda req: httpx.Response(403, text="<Error><Code>InvalidAccessKeyId</Code></Error>")
    )
    secrets = FakeSecrets(s=r2.credential_blob(AKID, SECRET))
    p = R2Provider(
        secrets=secrets,
...
def test_missing_credential_blob_is_a_clear_error():
    ...
    with pytest.raises(ScatterboxError, match="access key/secret"):
        R2Provider(secrets=secrets, secret_name="s", account_id=ACCOUNT, bucket=BUCKET)
