...
def test_builtin_registry_shape():
    # chaos hidden
    assert known_types() == [
        "dropbox", "gdrive", "koofr", "localfs", "onedrive", "pcloud", "vercel_blob"
    ]
    ...
    # Koofr (app password) and Vercel Blob (a read-write token) keep credentials
    # in the vault but are NOT OAuth: they require secrets yet have no
    # oauth_module, so they onboard via their own credential prompts, not the
    # loopback flow.
