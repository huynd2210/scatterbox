...
*Deviations:* snapshot locations are recorded inside the vault (which already holds the credentials), so recovery = vault + passphrase. ~~§9's alternate "re-auth one provider and find the snapshot by name" discovery path is not implemented~~ **Implemented 2026-06-12 (cold recovery):** snapshots are v2 (`SBSNAP2`: embedded non-secret Argon2id parameters) under a fixed object name; adapters gained an optional `find()`; recovery now also works with NOTHING local — passphrase + re-auth one provider (`scatterbox recover`, wizard "recover with passphrase", lone-.sbsnap import). The vault is recreated with the original salt so wrapped file keys still unwrap; `provider reauth` restores remaining OAuth credentials without touching replicas. Auto-snapshot (debounced ~20 s after mutations) runs in the daemon only; CLI users snapshot explicitly via `scatterbox snapshot`. The UI export is one zip containing the two §9 files; the first-run wizard offers set-up-new vs import (zip, vault+register, or vault-only → provider-snapshot recovery). Both gates are automated tests (test_portability.py, test_daemon.py) and were also exercised live end-to-end.

**Phase 5 — Policies, erasure coding, exotic adapters** ✅ **Complete 2026-06-11 (exotic adapters deferred by user decision)**
...
*Deviations:* exotic adapter *implementations* (Discord/YouTube/Mega/Pastebin…) explicitly deferred; instead the plug-in seams shipped — an adapter registry (`providers/__init__.py`: `AdapterSpec` + `register_adapter`, read by the factory, CLI, daemon, and OAuth onboarding alike) and a fully-commented skeleton (`providers/_template.py`) with the §6 profile priors for the planned types. EC: shares are stored as replica rows with `share_index`/`share_hash` (schema v6); `replica_target=n`, health learns the k death-threshold; repair regenerates exactly the missing share indices from any k survivors; `min_spread` is ignored for EC (anti-colocation is inherent at 1/k per provider). EC chaos gate automated (test_ec.py: kill n−k of 5 → byte-identical restores → repair on spares → healthy). Policies: deepest-ancestor folder resolution, explicit flags win field-by-field, identical across library/CLI/daemon/UI; Discord round-trip gate is N/A until that adapter lands.
...
9. ✅ Exotic adapter implementations deferred (user decision, Phase 5): the adapter registry + `providers/_template.py` are the integration seam — Discord/YouTube/Mega/Pastebin land later as one module + one `register_adapter` call each.
